  ...
  $http.get('http://localhost:8090/voyages').success(function(data,status,headers,config)
    {
      ...
      for(var i = 0; i < $scope.voyages.length; i++)
      {
        ...
        console.log("STATUS", statusArray[Math.round(Math.random())]);
        ...
        $scope.voyages[i].singleParameters = {
          ...
          wind : {label: "Vind", upperLimit: $scope.voyages[i].requiredMaxWindSpeed, current: $scope.voyages[i].currentWeatherWaypoint.currentSpeed, status: $scope.voyages[i].currentWeatherWaypoint.currentSpeedStatus, unit: "m/s"}
